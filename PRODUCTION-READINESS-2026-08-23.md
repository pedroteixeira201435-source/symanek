...
**Mock-only (8 modules)** — no `api.js` import, read `src/data.js` directly, no persistence path:
`Dashboard`, `Accounting`, `Library`, `POS`, `Scheduling`, `Exams`, `ApplyOnline`, `CanteenAdmin`.

**Caveat — "wired" ≠ "persists":** several api functions used by the "backed" modules are **pure mock with no http branch** (`api.js:112,276,337,426,445,446,455`): `getDegreeAudit`, `listExamSchedule`, `getCourseware`, `submitApplication`, `allocateRoom` (Accommodation write), `submitNcheReturn` (Compliance write), `setInstitutionType`. So Accommodation and Compliance *read* from backend but their *writes* are fake. And **all 13 still `import from '../data'`** for secondary datasets (charts/config), so `data.js` is not eliminable yet.
...
| A3 | Introduce real RLS privilege separation: split `is_admin()` from staff; gate write policies by `suite_role` (bursar↔finance, registrar↔students, etc.) so RLS matches `ROLE_NAV`. | L | **Yes** | ✅ **DONE (crown-jewel tables) 2026-08-23** — `has_suite_role()` + write policies scoped by role on payments/invoices/invoice_payments/applications/students/results/staff (`20260823140000`); verified by impersonation (bursar✗ students, librarian✗ payments, teacher✗ staff, admin✓). **Follow-up:** extend to accommodation/sponsors/exam_sittings/HR-contracts/compliance tables. |
...
| B4 | Purge demo numbers (476 etc.) from any prod-visible dashboard; drive Dashboard from real aggregates. | M | **Yes** if Dashboard is day-1 |
